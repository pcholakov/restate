    ...
    T: TransportConnect,
{
    pub fn update(&mut self, service: &Service<T>) -> anyhow::Result<()> {
        ...
        // ...provided it has a reasonably fresh view of metadata. Stale metadata might indicate
        ...
        match (is_leader, &self, fresh_metadata) {
            ...
            (true, ClusterControllerState::Follower, false) => {
                info!(
                    "It looks like I might be the leader, but my metadata is outdated so remaining a follower"
                );
            }
