...
use super::{
    ArchivedLsn, LocalPartitionSnapshot, PartitionSnapshotMetadata, SnapshotError,
...
impl SnapshotPartitionTask {
    ...
    pub async fn run(self) -> Result<(ArchivedLsn, PartitionSnapshotMetadata), SnapshotError> {
    ...
    async fn create_snapshot_inner(
        &self,
    ) -> Result<(ArchivedLsn, PartitionSnapshotMetadata), SnapshotError> {
        let snapshot = self
