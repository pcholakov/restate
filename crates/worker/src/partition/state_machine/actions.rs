...
use restate_types::identifiers::{
    ClusterSnapshotId, InvocationId, PartitionId, PartitionProcessorRpcRequestId,
};
...
pub enum Action {
    ...
    /// Tells the leadership layer to begin the local snapshot sequence:
    /// gate shuffle, flush SelfProposer, drain self-loop, take RocksDB
    /// checkpoint, send markers, self-propose LocalSnapshotTaken, release gate.
    BeginLocalSnapshot {
        snapshot_id: ClusterSnapshotId,
        num_partitions: u32,
    },
    /// All markers received and local snapshot taken — report completion
    /// to the snapshot coordinator.
    SnapshotComplete {
        snapshot_id: ClusterSnapshotId,
    },
    /// Send an outbox-processed ack to the source partition via Bifrost.
    /// Generated by the PP when processing a cross-partition message (both
    /// new and duplicate) to enable the sender to delete its outbox entry.
    SendOutboxAck {
        /// The partition that originally sent the message (ack target).
        to_partition: PartitionId,
        /// Our partition (the one that processed / dedup-rejected the message).
        from_partition: PartitionId,
        /// The exact outbox sequence number being acknowledged.
        seq: MessageIndex,
    },
}
